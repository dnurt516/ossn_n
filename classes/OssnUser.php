...
class OssnUser extends OssnEntities {
		...
		public function Login() {
				...
				if($user && ($this->password === $user->password || password_verify($this->password, $user->password)) && $user->activation == NULL) {
		...
		public function iconURL() {
				...
				foreach(ossn_user_image_sizes() as $size => $dimensions){
						...
						$params = array();
						$params['from'] = 'we_users';
						$params['params'] = array('avatar');
						$params['wheres'] = array('id = '.$this->guid);
						$cust_ava = $this->select($params);
						$cust_ava = arrayObject($cust_ava, $this);
						$url = "https://weplay.io/".(($cust_ava->avatar)?"files/avatars/".$cust_ava->avatar:"/design/img/themes/theme-1/avatar200.png");
		...
		public function profileURL($extends = '') {
				...
				$this->profileurl = "https://weplay.io/user/{$this->guid}" . $extends;

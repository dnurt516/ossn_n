...
class OssnObject extends OssnEntities {
		...
		public function addObject() {
				...
				if ($this->proj_id) {
					array_push($params['names'], 'proj_id'); 
					array_push($params['values'], $this->proj_id); 
				} elseif ($this->comp_id) {
					array_push($params['names'], 'comp_id'); 
					array_push($params['values'], $this->comp_id); 
				}
				...
				if($create) {
						if($this->insert($params)) {
								...
								if ($this->proj_id) {
									$args['proj_id'] = $params['values'][6]; 
								} elseif ($this->comp_id) {
									$args['comp_id'] = $params['values'][6];
								}
		...
		public function getObjectById(array $options = array()) {
				...
				$params['params'] = array(
 			    ...
 			    if ($options['subtype'] == 'ossngroup') {
  			      $params['params'] = array_merge($params['params'], array('o.proj_id', 'o.comp_id'));
   			    }

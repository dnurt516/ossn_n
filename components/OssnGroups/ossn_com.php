		...
		switch($page) {
				case 'add':
						$params = array(
								...
								'action' => 'https://weplay.io/ajax/group/add',
				...
				case 'add_cp':
						$params = array(
								// 'action' => ossn_site_url() . 'action/group/add',
								'action' => 'https://weplay.io/ajax/group/add_cp',
								'component' => 'OssnGroups',
								'class' => 'ossn-form'
						);
						$form   = ossn_view_form('add_cp', $params, false);
						echo ossn_plugin_view('output/ossnbox', array(
								'title' => ossn_print('add:group'),
								'contents' => $form,
								'callback' => '#ossn-group-submit'
 ...
 * @access private
 */
function ossn_group_page($pages, $ossn_user=NULL) {
		if(empty($pages[0])) {
				ossn_error_page();
		}
		if(!empty($pages[0])) {

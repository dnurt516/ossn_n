...
$file = ossn_file_get($params['post']->item_guid); 
?>
	...
	<div class="row">
		<div class="meta">
		   ...
		   if ($params['user']->guid == $params['post']->owner_guid || $params['post']->type == 'group') { ?>
            ...
            <?php } ?>
            <div class="ossn-wall-item-type"><?php echo ossn_print('post:shared:file:title');?></div>            
			</div>
			<div class="post-meta">
       ...
       <div class="post-contents post-share-wall-item">
				<?php 
				if($file){	
					$contents = ossn_plugin_view('files/item', array(
								'file' => $file,
								'user_entity' => false,
					));
					echo ossn_plugin_view('postshare/wrapper', array(
							...
							'user' => ossn_user_by_guid($file->owner_guid),
					));
				} else {

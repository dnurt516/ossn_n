...
$product = marketplace_get_product($params['post']->item_guid);
?>
	...
	<div class="row">
		<div class="meta">
		   ...
		   if ($params['user']->guid == $params['post']->owner_guid || $params['post']->type == 'group') { ?>
            ...
            <?php } ?>
            <div class="ossn-wall-item-type"><?php echo ossn_print('post:shared:marketplace:product');?></div>            
			</div>
			<div class="post-meta">
       ...
       <div class="post-contents post-share-wall-item">
				<?php 
				if($product){
					$contents = ossn_plugin_view('postshare/marketplace', array(
								'product' => $product,
					));
					echo ossn_plugin_view('postshare/wrapper', array(
							'contents' => $contents,
							'time_created' => $product->time_created,
							'user' => ossn_user_by_guid($product->owner_guid),
					));
				} else {

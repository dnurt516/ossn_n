...
function ossn_comment_menu($name, $type, $params) {
		...
		if($comment->type == 'comments:post') {
				if(com_is_active('OssnWall')) {
						...
						if($user->canModerate() || ($user->guid == $post->owner_guid) || ($comment && $user->guid == $comment->owner_guid) || ($group && $user->guid == $group->owner_guid)) {
								ossn_unregister_menu('delete', 'comments');
								ossn_register_menu_item('comments', array(
										'name' => 'delete',
										'href' => "https://weplay.io/ajax/delete/comment?comment={$params['id']}",
		...
		if($comment->type == 'comments:object') {
						...
						if($object){
							if($user->canModerate() || ($object->type == 'user' && $user->guid == $object->owner_guid) || $user->guid == $comment->owner_guid) {
								ossn_unregister_menu('delete', 'comments');
								ossn_register_menu_item('comments', array(
										'name' => 'delete',
										'href' => "https://weplay.io/ajax/delete/comment?comment={$params['id']}",
		...
		if($user && $comment->type == 'comments:entity') {
				$entity = ossn_get_entity($comment->subject_guid);
				if(($user->guid == $params['owner_guid']) || $user->canModerate() || ($comment->type == 'comments:entity' && $entity->type = 'user' && $user->guid == $entity->owner_guid)) {
						ossn_unregister_menu('delete', 'comments');
						ossn_register_menu_item('comments', array(
								'name' => 'delete',
								'href' => "https://weplay.io/ajax/delete/comment?comment={$params['id']}",
...
function ossn_comment_edit_menu($name, $type, $comment) {
		...
		$user = ossn_loggedin_user();
		if ($comment['weplay_user']) {
			$user = $comment['weplay_user'];
		}
		if(!empty($comment['id'])) {

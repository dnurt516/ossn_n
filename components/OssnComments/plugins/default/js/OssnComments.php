...
Ossn.PostComment = function($container) {
    ...
    if (window.location.href.includes('network')) {
        url_ajax = Ossn.site_url + 'action/post/comment';
    } else {
        url_ajax = 'https://weplay.io/ajax/post/comment';
    }
    Ossn.ajaxRequest({
        url: url_ajax,
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    $('#ossn-comment-attachment-' + $ftype + '' + $container).find('.image-data')
                        .html('<img loading="lazy" src="' + Ossn.site_url + 'components/OssnComments/images/loading.gif" style="width:30px;border:none;height: initial;" />');
                ...
                success: function(callback) {
                    if (callback['type'] == 1) {
                        ...
                        $('#ossn-comment-attachment-' + $ftype + '' + $container).find('.image-data')
                            .html('<img loading="lazy" src="' + Ossn.site_url + 'comment/staticimage?image=' + callback['file'] + '" />');
                    }
                    if (callback['type'] == 0) {
...
function ossn_comment_edit() {
    $(document).ready(function() {
        ...
        //comment edit
        Ossn.ajaxRequest({
            url: "https://weplay.io/ajax/comment/edit",
            containMedia: true,
            ...
            callback: function(callback) {
                if (callback['success']) {
                    ...
                    if ($elem) {
                        $elem.text('');
                        Ossn.PostRequest({
                            url: "https://weplay.io/ajax/comment/embed",
                            // params: 'content=' + $text,

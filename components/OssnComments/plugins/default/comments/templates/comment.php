 ...
 */
if ($params['weplay_user']) {
	$params['comment']['weplay_user'] = $params['weplay_user'];
}
		...
		<div class="col-11">
			<div class="comment-contents">
				<div class="row justify-content-between">
                    ...
                    <div class="ossn-comment-menu col-auto order-2">
                        <div class="dropdown">
                            <?php
                ...
                <p>
						...
						echo "<span class='comment-text'>";
						        if ($comment->type == 'comments:entity') {
						            echo ' '.html_entity_decode($comment->getParam('comments:entity'));
						        } elseif ($comment->type == 'comments:object') {
						            echo ' '.html_entity_decode($comment->getParam('comments:object'));
						        } elseif ($comment->type == 'comments:post') {
						            echo ' '.html_entity_decode($comment->getParam('comments:post'));
						        }
                        echo "</span>";
						?>
				</p>
				...
				<div class="comment-metadata">
					<div class="time-created col-auto order-1"><?php echo ossn_user_friendly_time($comment->time_created); ?></div>
					<?php
						if (class_exists('OssnLikes') && $allow_comment_like) {
						    ...
						    } elseif ($params['weplay_user']->guid) {
				             	 if (!$OssnLikes->isLiked($comment->id, $params['weplay_user']->guid, $type)) {
										echo ossn_plugin_view('output/url', array(
											'href' => "https://weplay.io/ajax/annotation/like?annotation={$comment->id}", 
											'text' => ossn_print('like'), 
											'class' => 'ossn-like-comment',
											'id' => 'ossn-like-comment-'.$comment->id,
											'data-type' => 'Like',
							 			...
							 			echo ossn_plugin_view('output/url', array(
											'href' => "https://weplay.io/ajax/annotation/unlike?annotation={$comment->id}", 
											'text' => ossn_print('unlike'), 
											'class' => 'ossn-like-comment',
											'id' => 'ossn-like-comment-'.$comment->id,									
											'data-type' => 'Unlike',

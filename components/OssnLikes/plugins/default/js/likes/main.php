...
Ossn.PostUnlike = function(post) {
	if (window.location.href.includes('network')) {
	    url_ajax = Ossn.site_url + 'action/post/unlike';
	} else {
	    url_ajax = 'https://weplay.io/ajax/post/unlike';
	}
    Ossn.PostRequest({
        url: url_ajax,
        beforeSend: function() {
...
Ossn.PostLike = function(post, $reaction_type = '') {
	if (window.location.href.includes('network')) {
	    url_ajax = Ossn.site_url + 'action/post/like';
	} else {
	    url_ajax = 'https://weplay.io/ajax/post/like';
	}
    Ossn.PostRequest({
        url: url_ajax,
        beforeSend: function() {
...
Ossn.RegisterStartupFunction(function() {								  
    ...
    $('body').on('click', '.ossn-like-comment-react, .ossn-like-comment', function(e) {
            ...
            Ossn.PostRequest({
                url: $url,
                action: false,
				params: '&'+$url.split('?')[1]+'&reaction_type='+$item.attr('data-reaction'),
                beforeSend: function() {
                ...
                callback: function(callback) {
                    if (callback['done'] == 1) {
                        ...
                        if ($type == 'Like') {
                            $('#ossn-like-comment-'+$total_guid).html('Unlike');
                        ...
                        if ($type == 'Unlike') {
                           $('#ossn-like-comment-'+$total_guid).html('Like');

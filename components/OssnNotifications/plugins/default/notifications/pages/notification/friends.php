    ...
    <div class="notification-friends">
        ...
        if ($params['friends']) {
            ...
            foreach ($params['friends'] as $users) {
                ...
                $img = "<img class='user-icon-small' style='width: 50px; display: inline-table; float: left;' src='{$users->iconURL()->small}' />";
		              ...
		              <div class='ossn-notifications-friends-inner'>
						...
						<a href='{$url}' class='user'>{$users->fullname}</a>
						  ...
						  <form id='add-friend-{$users->guid}'>
						   ...
						   <input class='btn btn-danger btn-sm' type='submit' value='{$denybutton}' />
						   </form>
                           </div>

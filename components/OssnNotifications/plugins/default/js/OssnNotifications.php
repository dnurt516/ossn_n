...
Ossn.NotificationFriendsShow = function($div) {
	$('div.ossn-notifications-box').css('display', 'block');
    ...
    Ossn.PostRequest({
        ...
        callback: function(callback) {
            ...
            Ossn.NotificationBox('success', data, 'firends', height);
        }
    });
};
...
Ossn.AddFriend = function($guid) {
    action = "https://weplay.io/ajax/friend/add?user=" + $guid;
    Ossn.ajaxRequest({
        url: action,
        ...
        callback: function(callback) {
            ...
            Ossn.NotificationsCheck();
            $('.notice_friend-request .notice_count').text(Number($('.notice_friend-request .notice_count').text())-1);
        }
    });
};

Ossn.removeFriendRequset = function($guid) {
    action = "https://weplay.io/ajax/friend/remove?user=" + $guid;
    Ossn.ajaxRequest({
        url: action,

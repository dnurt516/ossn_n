	...
	<div class="row">
		<div class="meta">
			...
			<div class="post-menu">
				<div class="dropdown">
                 <?php
           			if (ossn_is_hook('wall', 'post:menu') && (ossn_isLoggedIn() || $params['weplay_user'])) {
                		$menu['post'] = $params['post'];
                		$menu['weplay_user'] = $params['weplay_user'];
			...
			<div class="post-meta">
				<a class="time-created ossn-wall-post-time" title="<?php echo date('d/m/Y', $params['post']->time_created);?>" href="//<?php echo $_SERVER['HTTP_HOST'] . '/user/' . $params['post']->owner_guid . '/feed#activity-item-' . $params['post']->guid; ?>"><?php echo ossn_user_friendly_time($params['post']->time_created); ?></a>
        ...
        <div class="post-friends">
			 ...
			 <?php
				...
				if(!empty($params['friends'])){
					...
					if(!empty($friends)){
						echo '<div class="row post-friends-block"><i class="fa fa-user col-auto"></i><div class="friends col-auto">';
		...
		<div class="post-contents">
            ...
            <?php
                    if(!empty($params['text'])){
                        if (!empty($image)) {
                            ?>
                                <div class="post-contents_text"><p><?php echo $params['text']; ?></p></div>
                                <div class="post-contents_combo"><img src="<?php echo ossn_site_url("post/photo/{$params['post']->guid}/{$image}"); ?>"/></div>
                            <?php 
                        }
                    }   
              ...
              <?php
          			  if (ossn_is_hook('post', 'comments')){
                			...
                			$vars['post'] =  $params['post'];
                 		    $vars['user'] = $params['user'];					  
                 		    $vars['weplay_user'] = $params['weplay_user'];					  

...
function ossn_wall_post_edit(){
	$(document).ready(function(){
		//post edit
		Ossn.ajaxRequest({
			url: "https://weplay.io/ajax/wall/post/edit",
			containMedia: true,
			...
			callback: function(callback){
				if(callback['success']){
					...
					if($elem.length){
						$elem.text('');
						Ossn.PostRequest({
							url: "https://weplay.io/ajax/wall/post/embed/",
...
function ossn_wall_init(){
	$(document).ready(function(){
		$('.ossn-wall-container').find('.ossn-wall-friend').on('click', function(){
			// $('#ossn-wall-location .mapboxgl-ctrl').hide();
			// $('#ossn-wall-photo').hide();
			$('#ossn-wall-friend').show();
			$('#ossn-wall-friend .token-input-input-token').show();
		});
		$('.ossn-wall-location').on('click', function(){
			$('#ossn-wall-friend').hide();
			$('#ossn-wall-photo').hide();
			$('#ossn-wall-location').show();
		});
		$('.ossn-wall-container').find('.ossn-wall-photo').on('click', function(){
			$('#ossn-wall-friend .token-input-input-token').hide();
			// $('#ossn-wall-location .mapboxgl-ctrl').hide();
...
function ossn_wall_select_friends(){
	$(document).ready(function(){
		if(typeof $.fn.tokenInput === 'function'){
			$("#ossn-wall-friend-input").tokenInput("https://weplay.io/ajax/friendpicker", {
				placeholder: Ossn.Print('tag:friends'),
				hintText: false,
				propertyToSearch: "first_name",
				resultsFormatter: function(item){
					return "<li class='dropdown_full_name'>" + "<img src='" + item.imageurl + "' title='" + item.first_name + " " + item.last_name + "' height='40px' width='40px' />" + "<div class='full_name_in'><div class='full_name'>" + item.first_name + " " + item.last_name + "</div></div></li>"
				},

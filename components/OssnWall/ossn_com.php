			...
			if($post->poster_guid == $user->guid || $user->canModerate()) {
					$params = array(
							...
							'contents' => ossn_view_form('post/edit', array(
									'action' => 'https://weplay.io/ajax/wall/post/edit',
									'id' => 'ossn-post-edit-form',
									'component' => 'OssnWall',
									'params' => array(
											'post' => $post
...
function ossn_wall_post_menu($hook, $type, $return, $params) {
		...
		if ($params['weplay_user']) {
			$user = $params['weplay_user'];
		}
		if($params['post']->type == 'group') {
				$group = ossn_get_group_by_guid($params['post']->owner_guid);
		}
		if(($params['post']->poster_guid == ossn_loggedin_user()->guid || $params['post']->poster_guid == $params['weplay_user']->guid) || $params['post']->owner_guid == $user->guid || (isset($group) && ($group->owner_guid == $user->guid || $group->isModerator($user->guid))) /*|| $user->canModerate()*/) {
				
				$deleteurl = "https://weplay.io/ajax/wall/post/delete?post={$params['post']->guid}";
		...
		if(($params['post']->poster_guid == ossn_loggedin_user()->guid || $params['post']->poster_guid == $params['weplay_user']->guid /*|| $user->canModerate()*/) && empty($params['post']->item_guid)) {
...
function ossn_wall_view_template(array $params = array()) {
        global $ossn_user;
        // if (!$params['user']) $params['user'] = $ossn_user;
		if(!is_array($params)){
			return false;
		}

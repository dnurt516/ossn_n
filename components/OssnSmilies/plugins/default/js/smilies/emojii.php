Ossn.RegisterStartupFunction(function() {
	...
	$(document).ready(function() {
		...
		//	if (e.target.className != 'fa-fa-smile' && e.target.className != 'emojii') {
		//	if (e.target.className != 'ossn-wall-container-control-menu-emojii-selector') {
		//		$('#master-moji .emojii-container-main').hide();
		//	}
		// };

		$(window).on('click', function(e) {
			if ($(e.target).attr('class') != ' ossn-wall-container-control-menu-emojii-selector' && $(e.target).attr('class') != 'fa-fa-smile' && $(e.target).is('path') == false) {
				$('#master-moji .emojii-container-main').hide();
			} 
		});
		...
		// 1. comment
		$('body').on('click', '.ossn-comment-attach-photo .fa-face-smile', function(e) {
			$parent = $(this).parent().parent().parent();
			Ossn.OpenEmojiBox('#' + $parent.find('.comment-box').attr('id'));

			var comment_box_width = document.querySelector('.comment-box').offsetWidth;
			var comment_box_height = document.querySelector('.comment-box').offsetHeight;
			var emojii_cont_width = document.querySelector('.emojii-container').offsetWidth;
			var emojii_cont_height = document.querySelector('.emojii-container').offsetHeight;

			$('.emojii-container').css('top', 'calc('+$(this).parent().next('.comment-box').offset().top+'px + '+comment_box_height+'px)');
			$('.emojii-container').css('left', 'calc('+$(this).parent().next('.comment-box').offset().left+'px + '+comment_box_width+'px - '+emojii_cont_width+'px)');
		});
		...
		$('body').on('click', '.ossn-wall-container-control-menu-emojii-selector', function(e) {
			...
			var post_textarea_width = document.querySelector('.ossn-wall-container-data textarea').offsetWidth;
			var post_textarea_height = document.querySelector('.ossn-wall-container-data textarea').offsetHeight;
			var emojii_cont_width = document.querySelector('.emojii-container').offsetWidth;
			var emojii_cont_height = document.querySelector('.emojii-container').offsetHeight;

			$('.emojii-container').css('top', 'calc('+$(this).parent().parent().find('textarea').offset().top+'px + '+post_textarea_height+'px)');
			$('.emojii-container').css('left', 'calc('+$(this).parent().parent().find('textarea').offset().left+'px + '+post_textarea_width+'px - '+emojii_cont_width+'px)');
		});
